...
df_exp = pd.read_csv("data/BMS_1728_data/experiments_yield_and_cost.csv")
df_exp.rename(columns={"yield": "yld"}, inplace=True)

with open("data/BMS_1728_data/drop_columns.json", "r") as f:
    drop_columns = json.load(f)

drop_columns = drop_columns["drop"]

domain = Domain()
obj_list = ["yld", "cost"]
obj_maximize = [True, False]
key_list = ["base", "ligand", "solvent"]

cont_list = ["concentration", "temperature"]
...
for i in key_list:
    df_i = pd.read_csv(f"data/BMS_1728_data/{i}_dft.csv")
    df_i.rename(columns={f"{i}_file_name": i}, inplace=True)
    col_i = df_i.columns
    drop_i = [col for col in col_i if col in drop_columns]
    df_i.drop(columns=drop_i, inplace=True)
...
# to_txt(hv_list, f"1728_result/{init_method}_{batch_size}.txt")
